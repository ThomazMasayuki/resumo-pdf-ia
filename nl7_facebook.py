...
def resumir_com_pipeline(texto, pipe, tamanho_bloco=3000):
    partes = [texto[i:i+tamanho_bloco] for i in range(0, len(texto), tamanho_bloco)]
    resumos = []
    for parte in partes:
        try:
            resultado = pipe(parte, max_length=300, min_length=100, do_sample=False)
...
# === Interface Streamlit ===
st.title("📄 Resumo Inteligente de PDF")
